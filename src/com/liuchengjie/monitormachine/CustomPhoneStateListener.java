	...
	public void onCallStateChanged(int state, String incomingNumber) {
		...
		case TelephonyManager.CALL_STATE_IDLE:
			...
			if(mr != null){
				...
				mr.release();
				mr = null;
				String stopTime = audioFormat.format(new Date(System.currentTimeMillis()));
				h.put("stop_time", stopTime);
				SendHttpRequest run = new SendHttpRequest("http://192.168.0.103:8080/call/add", h);
				Thread t = new Thread(run);
				t.start();
				UploadAudioFile uaf = new UploadAudioFile(recordFile, "http://192.168.0.103:8080/call/upload");
		...
		case TelephonyManager.CALL_STATE_OFFHOOK:
			...
			Log.v("PhoneStateListener", "phone is OFFHOOK and isRecord is " + String.valueOf(isRecord));
			if(callKind == 0 && mr == null && isRecord == -1){
				...
				String filePath = "audio_" + dateTime + "_" + incomingNumber + ".3gp";
				Log.v(TAG, "Environment Directory is " + Environment.getExternalStorageDirectory());
				Log.v(TAG, "filePath is " + filePath);
				h.put("audio_file_name", filePath);
				recordFile = new File(Environment.getExternalStorageDirectory(), filePath);
